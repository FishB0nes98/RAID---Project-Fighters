// Hotshot Diablo Passive: Trickshot
...
class HotshotDiabloPassive {
    constructor() {
        this.passiveId = 'hotshot_diablo_trickshot';
        ...
        console.log('[HotshotDiabloPassive] Passive initialized');
    }

    initialize(character) {
        this.character = character;
        console.log(`[HotshotDiabloPassive] Initialize for ${character.name}`);
        ...
        console.log(`[HotshotDiabloPassive] Passive initialized for ${character.name}`);
    ...
    onDamageDealt(event) {
        ...
        console.log(`[HotshotDiabloPassive] Trickshot triggered from ${caster.name} dealing ${damage} damage to ${target.name} (Total triggers: ${this.trickshotTriggers})`);
    ...
    executeTrickshot(caster) {
        ...
        if (!gameManager) {
            console.error('[HotshotDiabloPassive] GameManager not found');
            return;
        }
        ...
        if (aliveEnemies.length === 0) {
            console.log(`[HotshotDiabloPassive] No alive enemies found for Trickshot`);
            return;
        }
    ...
    showTrickshotVFX(caster, target) {
        try {
            ...
            console.error('[HotshotDiabloPassive] Error creating Trickshot VFX:', error);
        }
    }
    ...
    addCharacterGlow(character) {
        try {
            ...
            console.error('[HotshotDiabloPassive] Error adding character glow:', error);
        }
    }
    ...
    createPassiveIndicator() {
        try {
            ...
            if (characterElement) {
                ...
                console.log(`[HotshotDiabloPassive] Created passive indicator for ${this.character.name}`);
            }
        } catch (error) {
            console.error('[HotshotDiabloPassive] Error creating passive indicator:', error);
        }
    }
    ...
    updatePassiveCounter() {
        try {
            ...
            console.error('[HotshotDiabloPassive] Error updating passive counter:', error);
        }
    }
    ...
    cleanup() {
        ...
        console.log(`[HotshotDiabloPassive] Cleaned up passive for ${this.character?.name || 'unknown'}`);
    }
}

// Register the passive globally
window.HotshotDiabloPassive = HotshotDiabloPassive;

// Register with PassiveFactory if available
if (window.PassiveFactory) {
    window.PassiveFactory.registerPassive('hotshot_diablo_trickshot', HotshotDiabloPassive);
    console.log('[HotshotDiabloPassive] Registered with PassiveFactory');
} else {
    console.warn('[HotshotDiabloPassive] PassiveFactory not found');
} 