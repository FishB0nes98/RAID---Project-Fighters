...
class Character {
    ...
    useAbility(index, target, options = {}) {
        ...
        // --- END MODIFIED MANA CHECK ---
         ...
         if (this.hasFreezeDebuff()) {
             // 58% chance for ability to succeed when frozen
             const successChance = Math.random();
             if (successChance < 0.58) {
        ...
        // --- NEW: Set current ability index for Zasalamel Scythe and other item effects ---
        this._currentAbilityIndex = index;
        const success = ability.use(this, target, actualManaCost);
        // Clear the ability index after use
        this._currentAbilityIndex = null;
        // --- END MODIFIED --- 
    ...
    applyDamage(amount, type, caster = null, options = {}) { // Added options object
        ...
        // Check for Shadow Dagger armor penetration enhancement
        let actualBypassArmor = bypassArmor;
        if (!actualBypassArmor && 
            window._shadowDaggerArmorPenetrationActive && 
            window._shadowDaggerEnhancedCharacter === caster &&
            type === 'physical') {
            actualBypassArmor = true;
            console.log(`[ShadowDagger] Automatically adding armor penetration for ${caster?.name}'s ability`);
        }
        
        console.log(`[DEBUG Destructure] Extracted from options:`, { isChainReaction, isRetaliation, bypassMagicalShield, bypassArmor: actualBypassArmor, abilityId });
        ...
        if (type === 'physical') {
            ...
            if (!actualBypassArmor) {
                ...
                if (this.stats.armor >= 0) {
                 ...
                 const log = window.gameManager ? window.gameManager.addLogEntry.bind(window.gameManager) : console.log;
                 const source = window._shadowDaggerArmorPenetrationActive ? 'Shadow Dagger enhanced ability' : 'Ability';
                 log(`${source} bypassed ${this.name}'s armor completely!`, 'passive'); 
            }
        ...
        if (actualDamageTaken > 0 && caster) {
            ...
            // Add ability index to options if available
            const enhancedOptions = { ...options };
            if (caster._currentAbilityIndex !== null && caster._currentAbilityIndex !== undefined) {
                enhancedOptions.abilityIndex = caster._currentAbilityIndex;
                console.log(`[Event Dispatch] Including ability index: ${caster._currentAbilityIndex}`);
            }
