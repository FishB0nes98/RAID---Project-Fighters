// Ability definition for Infernal Ibuki (Boss Version)

class InfernalIbukiBossCharacter extends Character {
...
const kunaiTossBossAbility = new Ability(
    'kunai_toss',
    'Kunai Toss',
...
const shadowStepBossAbility = new Ability(
    'shadow_step',
    'Shadow Step',
...
const dashingStrikeBossAbility = new Ability(
    'dashing_strike',
    'Dashing Strike',
...
document.addEventListener('DOMContentLoaded', () => {
    if (typeof CharacterFactory !== 'undefined' && CharacterFactory.registerCharacterClass) {
        CharacterFactory.registerCharacterClass('infernal_ibuki_boss', InfernalIbukiBossCharacter);
    } else {
        console.warn("InfernalIbukiBossCharacter defined but CharacterFactory not found or registerCharacterClass method missing.");
    }

    if (typeof AbilityFactory !== 'undefined' && AbilityFactory.registerAbilities) {
        // Register all Ibuki boss abilities
        AbilityFactory.registerAbilities([
            kunaiTossBossAbility,
            shadowStepBossAbility,
            dashingStrikeBossAbility // Add the new ability here
        ]);
    } else {
        console.warn("Ibuki boss abilities defined but AbilityFactory not found or registerAbilities method missing.");
        // Fallback
        window.definedAbilities = window.definedAbilities || {};
        window.definedAbilities.kunai_toss = kunaiTossBossAbility;
        window.definedAbilities.shadow_step = shadowStepBossAbility;
        window.definedAbilities.dashing_strike = dashingStrikeBossAbility;
    }
}); 